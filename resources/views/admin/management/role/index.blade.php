...
<script>
    ...
    $(document).ready(function(){
        dt = new DatatableCustomClass(`{{route('admin.management.role.datatables')}}`,`{{route('admin.management.role.datatablesColumn')}}`,target,$('[name*=search]'));
        dt.renderDatatables()
        ...
        $('#confrimationUpdateYes').on('click',function(){
            ...
            window.location.href = `{{route('admin.management.role.form')}}/${uuid}`
        })
        $('#confrimationDeleteYes').on('click',function(){
            var deleteMenu = new ajax(`{{route('admin.management.role.delete')}}`,{uuid : $(this).attr('data-id')},{},'DELETE').execAjax();            
