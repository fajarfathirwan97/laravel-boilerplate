...
<script>
    ...
    $(document).ready(function(){
        dt = new DatatableCustomClass(`{{route('admin.management.organization.datatables')}}`,`{{route('admin.management.organization.datatablesColumn')}}`,target,$('[name*=search]'));
        dt.renderDatatables()
        ...
        $('#confrimationUpdateYes').on('click',function(){
            ...
            window.location.href = `{{route('admin.management.organization.form')}}/${uuid}`
        })
        $('#confrimationDeleteYes').on('click',function(){
            var deleteMenu = new ajax(`{{route('admin.management.organization.delete')}}`,{uuid : $(this).attr('data-id')},{},'DELETE').execAjax();            
